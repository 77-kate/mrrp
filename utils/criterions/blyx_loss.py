...
def info_nce_loss(anchor, positive, negatives, tau=0.1):
    ...
    """
    # 合并正样本和负样本
    features = torch.cat([positive, negatives], dim=0)  # (N+1, d)
    
    # 计算相似度
    similarity = F.cosine_similarity(anchor, features, dim=-1)  # (N+1)
    
    # 计算损失
    logits = similarity / tau
    labels = torch.zeros(1, dtype=torch.long).to(anchor.device)
...
class BLYXLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.surv_criterion = CoxLoss()
        self.align_criterion = FocalLoss(num_class=2)
        print("Create BLYXLoss with align_criterion!")
    ...
    def forward(self, results):
        ...
        align_loss = self.align_criterion(cos_pred, cos_label)

        surv_loss = self.surv_criterion(results['risk_score'], results['rfs'], results['label'])
        total_loss = align_loss + surv_loss
       
        return surv_loss, align_loss, total_loss

class BLYXSurvLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.surv_criterion = CoxLoss()
        print("Create BLYXSurvLoss without align_criterion!")

    def forward(self, results):
        surv_loss = self.surv_criterion(results['risk_score'], results['rfs'], results['label'])
            
        return surv_loss, torch.tensor(0), surv_loss
