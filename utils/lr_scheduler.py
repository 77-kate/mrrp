...
class MultiStepLR(object):
    ...
    def step(self, epoch):
        return round(self.base_lr * (self.decayFactor ** bisect.bisect(self.steps, epoch)), 6)
...
def get_lr_scheduler(opts, printer=print):
    if opts.scheduler == 'multistep':
        step_size = opts.step_size if isinstance(opts.step_size, list) else [opts.step_size] # [120, 240]
        if len(step_size) == 1:
            step_size = step_size[0]
            step_sizes = [step_size * i for i in range(1, int(math.ceil(opts.epochs / step_size)))]
        else:
            step_sizes = step_size
        lr_scheduler = MultiStepLR(base_lr=opts.lr, steps=step_sizes, gamma=opts.lr_decay) # lr_decay=0.8 by default
...
def get_scheduler_opts(parser):
    ...
    group.add_argument('--scheduler', default='fixed', help='Learning rate scheduler (e.g. fixed, clr, poly)')
    group.add_argument('--step-size', default=None, type=int, nargs="+", help='Step sizes') # nargs="+": number of args could be one or more
    group.add_argument('--lr-decay', default=0.8, type=float, help='factor by which lr should be decreased')
