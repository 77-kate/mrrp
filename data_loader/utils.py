...
def load_lesions(data_dir, pid, lesion_height, lesion_width, img_extn, is_training, num_lesions=4, split="train"):
    ...
    # print(f'yx_img_path: {os.path.join(data_dir, pid, f"*.{img_extn}")} len(files): {len(files)}')
    files = [file for file in files if not file.endswith(f"mask.{img_extn}")]
    lesions = []
    keys = []
    lesions_label = []
    if len(files) == 0:
        print(data_dir, pid, ' no lesion img files be found.')

    if num_lesions > 0:
        files = np.random.choice(files, num_lesions, replace=True) # ensure the number of imgs per patient is the same as num_lesions

    for file in files:
        # lesion = sitk.GetArrayFromImage(sitk.ReadImage(file)) # dcm is 3D image with size (1,W,H)
        lesion = np.stack([
            sitk.GetArrayFromImage(sitk.ReadImage(file))[0],  # (1,W,H) -> (W,H)
            sitk.GetArrayFromImage(sitk.ReadImage(file))[0],
            sitk.GetArrayFromImage(sitk.ReadImage(file))[0],
        ], axis=-1) # (W,H,3)
        # print("after stack, yx Image array shape:", lesion.shape) 
        # lesion = np.stack([
        ...
        if is_training:
            lesion = random_transform_np(lesion, max_rotation=30, pad_value=0) # rotate the input image at random and fill blank area. return the transformed img.
    ...
    if len(lesions) == 0:
        print(data_dir, pid, "lesion img files exist but len(lesions) == 0") # debug
...
def load_all_bags_with_masks(data_dir, bag_height, bag_width, img_extn, img_mask_extn, num_bags, split="train"):
    ...
    for file in files:
        ...
        # (B_H, B_W, C)
        bag = openslide.OpenSlide(file).get_thumbnail((bag_width, bag_height)).convert("RGB")
        # bag.save(os.path.join(data_dir, os.path.basename(file).split('.')[0]+"test.jpg"), "JPEG")
        bag = np.array(bag)
        ...
        mask_file = file.replace("GuoZhongBingLiDataWB", "GuoZhongBingLiData").replace(f'-img.{img_extn}', f'-mask.{img_mask_extn}')
        mask = np.array(Image.open(mask_file).convert('L')) # np.asarray() return a view of input data, maybe read-only.
        # print(f'pid: {pid} file: {file} mask_file: {mask_file}')

        # histogram, bins = np.histogram(cv2.imread(mask_file, cv2.IMREAD_GRAYSCALE), bins=256, range=(0, 256))
        ...
        # plt.savefig(mask_file.replace(f'-mask.{img_mask_extn}', f'-maskGrayDist.jpg'), format='jpg', dpi=300)
        # plt.close()
        ...
        keys.append(os.path.basename(file).split('.')[0].replace("-img", "").replace("-mask", ""))
    if len(bags) == 0:
        return torch.Tensor(1)
...
def ccd_bl_her2(x):
    ...
    if str(x) != str(np.nan):
        if x.startswith("none"):
            her2[int(x[8:9])] = 1.0 # e.g. none（all3+）
        elif x.startswith("heterogeneity"): # e.g. heterogeneity（3+25%，2+5%，1+5%，0+65%）
            for t in x.split('（')[1][:-1].split("，"):
                try:
...
def ccd_time(x):
    x = x.split('-')[0] # get the year of time ('2018-07-26 00:00:00')
    # 患者：开始治疗时间
...
def ccd_fenxing(x):
    # print(f'fenxing: x.type: {type(x)} value: {x}') # x.type: <class 'int'>
    x=str(x)
    # 病理：LAUREN分型
    if x == "0": # Intestinal
        return torch.FloatTensor([1.0, 0.0, 0.0])
...
def ccd_fenhua(x):
    # print(f'fenhua: {x} x.type: {type(x)} ') # x.type: <class 'int'>
    x=str(x)
    # 病理：分化程度
    if x in ["0"]: # "低分化"
